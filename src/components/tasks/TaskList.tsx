...
export const TaskList = ({ tasks, hasEditPermission }: TaskListProps) => {
  ...
  return (
    <>
      <h2 className='mt-10'>Tareas</h2>
      <div className='flex flex-col md:flex-row gap-5 overflow-x-scroll 2xl:overflow-auto pb-20'>
        <DndContext onDragEnd={handleDragEnd}>
          {Object.entries(groupedTasks).map(([status, tasks]) => (
            <div key={status} className='min-w-[300px] 2xl:min-w-0 2xl:w-1/5'>
              <h3
                className={`text-sm md:text-base border border-slate-200 p-3 my-5 bg-white border-t-8 ${statusStyles[status]} rounded`}
              >
              ...
              <ul className='space-y-5'>
                {tasks.length === 0 ? (
                  <li className='text-sm sm:text-base text-slate-500 text-center mt-10 font-bold'>
